...
: G4UserSteppingAction(),fcount_ions(0)
{
...
/*    G4int count_ions=0;
  ...
  */
   ...
   // analysisManager->CreateH1("Anum","AtomicNumber", 100, 0., 100);
    
    //G4String fileName = "B4-Calor-2";
    ...
    analysisManager->FillH1(0, aStep->GetTotalEnergyDeposit());
    analysisManager->FillNtupleDColumn(0, aStep->GetTotalEnergyDeposit());
        ...
        for (size_t lp=0; lp<(*secondary).size(); lp++) {
            ...
            if (particle->GetParticleType() == "nucleus") {
                ...
                fcount_ions++;
                G4cout <<"\n number of ions: " << fcount_ions << " " << name << G4endl;
                //atomic mass
         ...
         for (size_t lp=0; lp<(*secondary).size(); lp++) {
            ...
            if (name == "e-") {
                fcount_electron++;
                G4cout <<"\n number of electrons: " << fcount_electron << " " << name << G4endl;
            if (name == "gamma") { 
                fcount_gamma++; 
